...
public class CandidateGenerator implements Serializable
{
    ...
    public String getCorrectedQuery(String query) throws Exception
    {
        ...
        {
            candidates
                    .put(tokens[i].trim(), lm.getCloseWords(tokens[i].trim()));
        }

        String str = getCartesianProducts(candidates,
                new String[candidates.size()], 0, tokens, query);
        currentQuery = "";
        currentProb = -1.0d / 0;
        return str.trim();
    }
            ...
            String[] current, int depth, String[] tokens, String q)
    {
        ...
        while (iter.hasNext())
        {
            ...
            if (depth < candidates.keySet().size() - 1)
            {
                ...
                {
                    distance = distance
                            + UniformCostModel.getEditDistance(current[i],
                                    tokens[i]);
                    if (distance > 2)
                    {
                        current[depth] = null;
            ...
            else
            {
                ...
                Arrays.stream(current).sequential()
                        .forEach(st -> r.append(st).append(" "));

                if (UniformCostModel.getEditDistance(r.toString().trim(), q
                        .toString().trim()) > 2)
                {
                    return null;
                }

                double prob = Math.log10(RunCorrector.nsm.ecm_.editProbability(
                        q.toString(), r.toString(), 1));
                prob = prob
                        + param
                        * Math.log10(RunCorrector.languageModel
                                .getUnigramProbability(current[0]));
                for (int i = 0; i < current.length - 2; i++)
                {
                    prob = prob
                            + param
                            * Math.log10(RunCorrector.languageModel
                                    .getBigramProbability(current[i],
                                            current[i + 1], LAMBDA));
                }
